...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_lists_all_bookinstances(self):
        for book in BookInstance.objects.all():
            book.status = 'o'
            book.save()
...
class RenewBookInstancesViewTest(TestCase):
    def setUp(self):
        ...
        permission1 = Permission.objects.get(name='Can renew due date')
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_login_with_permissions(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_form_date_of_death_initial_is_12_10_2016(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
        initial_date_of_death = datetime.date(year=2016, month=10, day=12)
        self.assertEqual(
            response.context['form'].initial['date_of_death'], initial_date_of_death)

    def test_redirects_to_new_author_page_on_success(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        test_author_first_name = 'John'
        test_author_last_name = 'Smith'
        test_author_date_of_birth = datetime.date(year=1980, month=10, day=1)
        test_author_date_of_dead = datetime.date(year=2000, month=10, day=1)
        response = self.client.post(reverse('author_create'), {
                                    'first_name': test_author_first_name,
                                    'last_name': test_author_last_name,
                                    'date_of_birth': test_author_date_of_birth,
                                    'date_of_death': test_author_date_of_dead}
                                    )
        author = Author.objects.first()
        self.assertRedirects(response, reverse(
            'author-detail', kwargs={'pk': author.pk}))
